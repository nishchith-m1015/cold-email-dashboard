...
SELECT
  ...
  -- Email step breakdown (extract from metadata if email_number doesn't exist)
  COALESCE(
    COUNT(*) FILTER (WHERE event_type = 'sent' AND (metadata->>'email_number')::int = 1),
    0
  ) AS email_1_sends,
  COALESCE(
    COUNT(*) FILTER (WHERE event_type = 'sent' AND (metadata->>'email_number')::int = 2),
    0
  ) AS email_2_sends,
  COALESCE(
    COUNT(*) FILTER (WHERE event_type = 'sent' AND (metadata->>'email_number')::int = 3),
    0
  ) AS email_3_sends,
